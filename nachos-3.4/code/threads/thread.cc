...
Thread::Thread(const char* threadName)
{
    ...
    // ---------- SCHEDULING ALGORITHM FIELDS INITIALIZATION ----------
    // Hybrid MLQ (RR + FCFS)
    rrQuantaUsed = 0;
    inRRQueue = true;
    ...
    waitingTime = 0;
    serviceTime = 0;      // will be set by test program (e.g., burst time)
    
    // Lottery Scheduling
    tickets = 1;          // default 1 ticket per thread (can be changed)
    
    // Preemptive Priority Scheduling
    priority = 0;         // 0 = highest priority (can be changed)
    
    // Multi-Level Queue (MLQ) – non‑feedback version
    currentQueueLevel = 0;     // start in highest priority queue
    ticksInCurrentQueue = 0;
