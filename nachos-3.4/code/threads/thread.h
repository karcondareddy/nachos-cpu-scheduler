...
class Thread {
  ...
  public:
    ...
    // Hybrid MLQ (RR + FCFS)
    int rrQuantaUsed;      // number of full RR quanta consumed (used for demotion)
    bool inRRQueue;        // true if currently in RR queue, false if in FCFS queue
    
    // HRRN (Highest Response Ratio Next)
    int waitingTime;       // total time spent waiting in ready queue
    int serviceTime;       // total CPU burst time (needed for ratio calculation)
    ...
    // Multi-Level Queue (MLQ) – non‑feedback version
    int currentQueueLevel;     // which priority queue the thread belongs to (0 = highest)
    int ticksInCurrentQueue;   // how many ticks executed in current queue (for possible demotion)
    
    // I/O Simulation (blocking/waiting)
    int sleepWakeupTime;       // absolute tick count when a sleeping thread should be woken
